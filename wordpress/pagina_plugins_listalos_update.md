...
``` javascript
    ...
    jQuery('.wp-list-table.plugins tbody tr:not(.plugin-update-tr)').each(function( index ) {
        ...
        pluginslist.push({
            ...
            "Types"             : classes,
            "Path"              : pathPlugin,
            "Atualizar?"        : isUpdate, // É uma atualização sim ou não?
            "Pago?"             : isPay, // Precisa da chave key para atualizar
            "Plugin Atualizado" : title,
            "Atual"             : versionNumber[1] ? 'v'+versionNumber[1] : '-',
            "Novo"              : isUpdate ? 'v' + jQuery.trim(newVersion.slice(-1)) : '',
            "Status"            : '-',
            "Observações"       : isPay ? 'Para atualizar precisa da chave de ativação' : '',
        });
